...
**Args:**
 ...
 - <b>`input_size`</b> (int, optional):  Input size for each window, if not none rolled windows. Defaults to None. 
 - <b>`level`</b> (List[float], optional):  Confidence levels between 0 and 100 for prediction intervals. Defaults to None. 
 - <b>`refit`</b> (bool or int):  Wether or not refit the model for each window. If int, train the models every `refit` windows. Defaults to True. 
 - <b>`fitted`</b> (bool):  Store in-sample predictions. Defaults to False. 
