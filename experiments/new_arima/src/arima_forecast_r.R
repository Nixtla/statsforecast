...
horizon <- meta[[args[2]]][['horizon']]
seasonality <- meta[[args[2]]][['seasonality']]

df <- read_csv(str_glue('data/{args[1]}-{args[2]}.csv'))
...
forecasts %>% 
  write.table(
    str_glue('data/arima-r-forecast-{args[1]}-{args[2]}.txt'),
    row.name=F, col.name=F
  )

tibble(time=difftime(end, start, units="secs"), model='auto_arima_r') %>%
  write_csv(str_glue('data/forecast-arima-r-time-{args[1]}-{args[2]}.csv'))
