...
def main(dataset: str = 'M3', group: str = 'Other') -> None:
    ...
    models = [AutoETS(season_length=seasonality)]
    ...
    forecasts.to_csv(f'data/statsforecast-forecasts-{dataset}-{group}-pred-int.csv', index=False)

    time_df = pd.DataFrame({'time': [end - start], 'model': ['ets_statsforecast']})
    time_df.to_csv(f'data/statsforecast-time-{dataset}-{group}-pred-int.csv', index=False)
