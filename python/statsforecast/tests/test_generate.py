...
class TestGenerateCesSamples:
    """Tests for the low-level generate_ces_samples helper function."""

    def test_basic_non_seasonal(self):
        """Test generate_ces_samples with non-seasonal (N) model."""
        np.random.seed(42)
        y = 100 + np.random.randn(50) * 5
        y = y.astype(np.float64)

        model = auto_ces(y, m=1)
        samples = generate_ces_samples(model, h=12, n_samples=100, random_state=42)

        assert samples.shape == (100, 12)
        assert not np.any(np.isnan(samples))

    def test_seasonal(self):
        """Test generate_ces_samples with seasonal model."""
        np.random.seed(42)
        t = np.arange(120)
        y = 100 + 0.5 * t + 10 * np.sin(2 * np.pi * t / 12) + np.random.randn(120) * 2
        y = y.astype(np.float64)

        model = auto_ces(y, m=12)
        samples = generate_ces_samples(model, h=12, n_samples=100, random_state=42)
    ...
    def test_reproducibility(self):
        ...
        samples1 = generate_ces_samples(model, h=12, n_samples=50, random_state=123)
        samples2 = generate_ces_samples(model, h=12, n_samples=50, random_state=123)

        np.testing.assert_allclose(samples1, samples2)


class TestAutoCESGenerate:
    """Tests for the AutoCES.generate() method."""
    ...
    def test_generate_seasonal(self):
        ...
        model = AutoCES(season_length=12)
        model.fit(y)
