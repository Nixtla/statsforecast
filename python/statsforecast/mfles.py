...
from ._lib import mfles as _mfles
...
def get_basis(y, n_changepoints, decay=-1, gradient_strategy=0):
    ...
    decay_val = float('nan') if decay is None else float(decay)
    return _mfles.get_basis(y, n_changepoints, decay_val, int(gradient_strategy))
...
def siegel_repeated_medians(x, y):
    return _mfles.siegel_repeated_medians(
