"""
Tests for UCM (Unobserved Components Model) module.
"""
import numpy as np
import pytest
...
def test_ucm_with_seasonal():
    ...
    y = 50 + seasonal + np.random.randn(n) * 2
...
def test_ucm_prediction_intervals():
    ...
    # Check that wider CI contains narrower CI
    assert np.all(forecast['lo-95'] <= forecast['lo-80'])
    assert np.all(forecast['hi-95'] >= forecast['hi-80'])
