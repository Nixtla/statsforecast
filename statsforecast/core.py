...
__all__ = ["StatsForecast", "ParallelBackend", "forecast", "cross_validation"]
...
class GroupedArray:
    ...
    def compute_forecasts(self, h, func, xreg=None, level=None, *args):
        ...
        if has_level:
            out = np.full(
                (h * self.n_groups, 2 * len(level) + 1), np.nan, dtype=np.float32
            )
...
class StatsForecast:
    ...
    def forecast(self, h, xreg=None, level=None):
        if xreg is not None:
            ...
            if xreg.shape != expected_shape:
                raise ValueError(
                    f"Expected xreg to have shape {expected_shape}, but got {xreg.shape}"
                )
    ...
    def cross_validation(
        self, h, n_windows=1, step_size=1, test_size=None, input_size=None
    ):
    ...
    def _sequential(
        self, h, test_size, step_size, input_size, xreg, level, mode="forecast"
    ):
        fcsts = {}
        ...
        for model_args in self.models:
            ...
            if mode == "forecast":
                ...
                values, test_values = self.ga.compute_cv(
                    h, test_size, model, step_size, input_size, *args
                )
                keys = None
            if keys is not None:
                for j, key in enumerate(keys):
