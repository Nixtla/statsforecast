...
# %% ../nbs/src/ets.ipynb 7
_LIB.ETS_Update.argtypes = [
    ctypes.POINTER(ctypes.c_double),  # l
    ctypes.POINTER(ctypes.c_double),  # b
    ctypes.POINTER(ctypes.c_double),  # s
    ctypes.c_double,  # old_l
    ctypes.c_double,  # old_b
    ctypes.POINTER(ctypes.c_double),  # old_s
    ctypes.c_int,  # m
    ctypes.c_int,  # trend
    ctypes.c_int,  # season
...
_LIB.ETS_Calc.argtypes = [
    ...
    ctypes.POINTER(ctypes.c_double),  # a_mse
    ctypes.c_int,  # n_mse
    ctypes.c_int,  # m
]
...
_LIB.ETS_NelderMead.argtypes = [
    ...
    ctypes.POINTER(ctypes.c_double),  # y
    ctypes.c_size_t,  # n_y
    ctypes.c_int,  # n_state
    ctypes.c_int,  # error
    ctypes.c_int,  # trend
    ctypes.c_int,  # season
    ctypes.c_int,  # opt_crit
    ctypes.c_int,  # n_mse
    ctypes.c_int,  # m
    ctypes.c_bool,  # opt_alpha
    ctypes.c_bool,  # opt_beta
    ctypes.c_bool,  # opt_gamma
    ctypes.c_bool,  # opt_phi
    ctypes.c_double,  # alpha
    ctypes.c_double,  # beta
    ctypes.c_double,  # gamma
    ctypes.c_double,  # phi
    ctypes.POINTER(ctypes.c_double),  # lower
    ctypes.POINTER(ctypes.c_double),  # upper
    ctypes.c_double,  # tol_std
    ctypes.c_int,  # max_iter
    ctypes.c_bool,  # adaptive
]
...
def etssimulate(
    ...
    s = np.zeros(24)
    f = np.zeros(10)
    if m > 24 and season > Component.NONE:
        return
    elif m < 1:
        m = 1
    ...
    if season > Component.NONE:
        for j in range(m):
            s[j] = x[(trend > Component.NONE) + j + 1]
    for i in range(h):
        # Copy previous state
        oldl = l
        if trend > Component.NONE:
        ...
        _LIB.ETS_Forecast(
            oldl,
            oldb,
            _arr_as_double_ptr(olds),
            m,
            trend,
            season,
            phi,
            _arr_as_double_ptr(f),
            1,
        )
...
def etsforecast(
    ...
    l = x[0]
    b = 0.0
    has_trend = trend > Component.NONE
    if has_trend:
        b = x[1]
    if season > Component.NONE:
...
def switch_criterion(x: str) -> Criterion:
    ...
    if x == "amse":
        return Criterion.AMSE
    if x == "sigma":
        return Criterion.SIGMA
    if x == "mae":
...
def pegelsresid_C(
    ...
    lik = _LIB.ETS_Calc(
        _arr_as_double_ptr(y),
        y.size,
        _arr_as_double_ptr(x),
        switch(errortype),
        switch(trendtype),
        switch(seasontype),
        alpha,
        ...
        gamma,
        phi,
        _arr_as_double_ptr(e),
        _arr_as_double_ptr(amse),
        nmse,
        m,
    )
    x = x.reshape((n + 1, p))
...
def optimize_ets_target_fn(
    ...
    if seasontype == "N":
        gamma = 0.0
    res = _LIB.ETS_NelderMead(
        _arr_as_double_ptr(x0),
        x0.size,
        _arr_as_double_ptr(y),
        y.size,
        nstate,
        ...
        gamma,
        phi,
        _arr_as_double_ptr(lowerb),
        _arr_as_double_ptr(upperb),
        1e-4,
        1_000,
        True,
    )
