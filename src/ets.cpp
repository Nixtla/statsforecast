...
void Update(double &l, double &b, std::vector<double> &s, double old_l,
            double old_b, const double *old_s, int m, Component trend,
            Component season, double alpha, double beta, double gamma,
            double phi, double y) {
  double q, phi_b;
  // new level
  ...
  if (season > Component::None) {
    ...
    std::copy(old_s, old_s + m - 1, s.begin() + 1);
  }
}
...
double ETS_Calc(const double *y, size_t n, double *x, Component error,
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    // update state
    ETS::Update(l, b, s, old_l, old_b, old_s.data(), m, trend, season, alpha,
                beta, gamma, phi, y[i]);
    // store new state
    x[n_states * (i + 1)] = l;
    if (trend > Component::None) {
