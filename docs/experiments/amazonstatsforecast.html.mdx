...
found [here](./amazonstatsforecast.html).
...
-   `models`: a list of models. Select the models you want from
    [models](../../src/core/models.html) and import them. For this
    example, we will use `AutoETS` and `DynamicOptimizedTheta`. We set
    `season_length` to 7 because we expect seasonal effects every week.
    (See: [Seasonal
