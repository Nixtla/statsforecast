...
[StatsForecast](../../index)’s probabilistic models.
...
[MSTL](../models/MultipleSeasonalTrend) model, which is well-suited for
low-frequency data like the one used here. We first need to import it
from `statsforecast.models` and then we need to instantiate it. Since
we’re using hourly data, we have two seasonal periods: one every 24
hours (hourly) and one every 24\*7 hours (daily). Hence, we need to set
...
any [probabilistic model](../../models) from StatsForecast can be used.
We also selected the 99% prediction interval of the insample forecasts,
but other confidence levels can be used as well.
