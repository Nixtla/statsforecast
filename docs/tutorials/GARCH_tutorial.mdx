...
implement a GARCH or an ARCH model in [StatsForecast](../../index) and
how they can be used to analyze and predict financial time series data.
...
We first need to import the [GARCH](../models/GARCH) and the
[ARCH](../models/ARCH) models from `statsforecast.models`, and then we
need to fit them by instantiating a new StatsForecast object. Notice
that we’ll be using different values of $p$ and $q$. In the next
section, we’ll determine which ones produce the most accurate model
using cross-validation. We’ll also import the
[Naive](../../models#class-naive) model since we’ll use it as a
baseline.
